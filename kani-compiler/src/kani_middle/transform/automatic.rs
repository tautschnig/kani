...
use crate::kani_middle::{
    CtorReturn, FmtTrait, SmartPointerModels, can_derive_arbitrary, find_arbitrary_constructor,
    fmt_impl_self_ty, implements_arbitrary, implements_invariant, scalar_niche,
...
pub struct AutomaticArbitraryPass {
    ...
    /// The (optional) FnDef of KaniModel::AnyVecIntoIter (requires alloc).
    kani_any_vec_into_iter: Option<FnDef>,
...
impl AutomaticArbitraryPass {
    pub fn new(_unit: &CodegenUnit, query_db: &QueryDb) -> Self {
        ...
        let kani_any_vec_into_iter = kani_fns.get(&KaniModel::AnyVecIntoIter.into()).copied();
...
impl TransformPass for AutomaticArbitraryPass {
    ...
    fn transform(&mut self, tcx: TyCtxt, body: Body, instance: Instance) -> (bool, Body) {
        ...
        if let TyKind::RigidTy(RigidTy::Adt(def, args)) = ty.kind() {
            // Under --constructor-args, generate values of structs with private fields through
            // one of their public constructors (raw field synthesis can violate the type's
            // representation invariant, producing false alarms); fall through to field
            // synthesis when no viable constructor exists.
            if self.constructor_args
                && def.kind() == AdtKind::Struct
                && crate::kani_middle::adt_has_private_field_check(tcx, def)
            {
...
fn call_kani_any_for_ty(
        ...
        )
    {
            ...
            }
        {
            let model_inst =
                Instance::resolve(unbounded_def, &GenericArgs(vec![GenericArgKind::Type(elem_ty)]))
                    .unwrap();
            let slice_lcl = body.new_local(ty, source.span(body.blocks()), mutability);
            body.insert_call(
                &model_inst,
                source,
                InsertPosition::Before,
        ...
        } else if let Some(model_inst) = kani_any_vec_into_iter.and_then(|model| {
            let elem = crate::kani_middle::vec_into_iter_elem_ty(ty)?;
            if !crate::kani_middle::slice_elem_unbounded_ok(tcx, elem) {
                return None;
            }
            Instance::resolve(model, &GenericArgs(vec![GenericArgKind::Type(elem)])).ok()
        }) {
            // std::vec::IntoIter<T> (Iterator-bound instantiations): unbounded generation,
            // c.f. the AnyVecIntoIter model (mirrors the eligibility decision).
            model_inst
...
impl AutomaticArbitraryPass {
    ...
    fn generate_ctor_body(
        ...
        // Some = variant 1 of Option; Ok = variant 0 of Result. Both have discriminant values
        // equal to their variant indices.
