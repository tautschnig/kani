...
pub struct CommonAutoharnessArgs {
    ...
    /// state. Failures are reported as a distinct property class ("mined invariant ...
    /// violated by return value"); since the mined predicate is heuristic, a failure means
    /// the returned value would trip the type's own assertions when used, which may or may
    /// not be a bug in the returning function.
    #[arg(long)]
