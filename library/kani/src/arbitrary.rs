...
/// Nondeterministic iterator for instantiating `Iterator`-bounded type parameters of
/// automatic harnesses: the parameter is instantiated with `std::vec::IntoIter<T>`, and the
/// argument generated by this model — an iterator over an *unbounded* nondeterministic
/// vector (c.f. `any_vec_unbounded`), so verification results hold for iterators of all
/// lengths and contents. Optional: requires `alloc`.
#[kanitool::fn_marker = "AnyVecIntoIterModel"]
#[inline(never)]
#[doc(hidden)]
pub fn any_vec_into_iter<T>() -> std::vec::IntoIter<T> {
    any_vec_unbounded::<T>().into_iter()
}
